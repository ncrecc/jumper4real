...
function level:init(leveldata, filepath, entrance) --1st arg must be actual level data. when game loads levels by doing love.filesystem.read, that's now on its end. previous comment: 'arg can be actual level data, e.g. level contents generated by editor and then tested, or a name pointing to a file with level data, e.g. "mycoollevel". the latter is what we do everywhere outside of the editor, and the string gets stored as the "filename" field. we check if it's a name by seeing if it contains no newlines'
